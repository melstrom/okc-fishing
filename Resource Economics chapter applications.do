...
sum income if income!=.
replace income = r(mean) if missing(income) ///empty cells are replaced with the average of nonmissing values
...
gen allelse = (waterbody=="Other")
replace logacres = 0 if allelse==1
replace distance = 50 if allelse==1
replace oklahomacity = 0 if allelse==1
gen tcost = 2*(distance*.2837+(distance/40)*(income/2000)/2)
sum dist if choice==1
drop if distance>r(mean)+2*r(sd)

clogit choice tcost catfish sunfish bass logacres oklahomacity pond allelse, group(choice_id) cluster(angler_id)

preserve
********************************************
***********Closing Lake Hefner**************
********************************************
gen exp_v = exp(_b[tcost]*tcost + _b[catfish]*catfish + _b[sunfish]*sunfish + _b[bass]*bass + _b[logacres]*logacres + _b[oklahomacity]*oklahomacity + _b[pond]*pond + _b[allelse]*allelse)
by choice_id, sort: egen sumv = sum(exp_v)
gen s = exp_v/sumv
gen exp_vstar = exp(_b[tcost]*tcost + _b[catfish]*catfish + _b[sunfish]*sunfish + _b[bass]*bass + _b[logacres]*logacres + _b[oklahomacity]*oklahomacity + _b[pond]*pond + _b[allelse]*allelse) if waterbody!="Hefner"
by choice_id, sort: egen sumvstar = sum(exp_vstar)
...
by choice_id, sort: egen sumv = sum(exp_v)
gen s = exp_v/sumv
gen exp_vstar = exp(_b[tcost]*tcost + _b[catfish]*catfish + _b[sunfish]*sunfish + _b[bass]*bass + _b[logacres]*logacres + _b[oklahomacity]*oklahomacity + _b[pond]*pond + _b[allelse]*allelse) if waterbody!="Stanley Draper"
by choice_id, sort: egen sumvstar = sum(exp_vstar)
gen sstar = exp_vstar/sumvstar
...
gen exp_vstar = exp(_b[tcost]*tcost + _b[catfish]*catfish + _b[sunfish]*sunfish + _b[bass]*bass + _b[logacres]*logacres + _b[oklahomacity]*oklahomacity + _b[pond]*pond + _b[allelse]*allelse) if okcpond==0
by choice_id, sort: egen sumvstar = sum(exp_vstar)
gen sstar = exp_vstar/sumvstar
table waterbody if oklahomacity==1, c(m s m sstar)
gen wtp = (ln(sumvstar) - ln(sumv))/(-1*_b[tcost])
...
gen exp_v = exp(_b[tcost]*tcost + _b[catfish]*catfish + _b[sunfish]*sunfish + _b[bass]*bass + _b[logacres]*logacres + _b[oklahomacity]*oklahomacity + _b[pond]*pond + _b[allelse]*allelse)
by choice_id, sort: egen sumv = sum(exp_v)
gen exp_vstar = exp(_b[tcost]*tcost + _b[catfish]*catfish + _b[sunfish]*sunfish*1.25 + _b[bass]*bass + _b[logacres]*logacres + _b[oklahomacity]*oklahomacity + _b[pond]*pond + _b[allelse]*allelse)
by choice_id, sort: egen sumvstar = sum(exp_vstar)
gen wtp = (ln(sumvstar) - ln(sumv))/(-1*_b[tcost])
sum wtp
restore
